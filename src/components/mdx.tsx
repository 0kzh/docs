  ...
  return (
    <li className="m-0 px-0 py-4 first:pt-0 last:pb-0">
      <dl className="m-0 flex flex-wrap items-center gap-x-3 gap-y-2">
        ...
        <dd className="text-green-700 font-bold font-mono">
          {name}
        </dd>
        {type && (
          <div className="flex gap-x-1">
            ...
            {Array.isArray(type) ? (
              type.map((t, index) => (
                <dd key={index} className={`font-mono text-xs ${t === 'required' ? 'text-red-500' : 'text-zinc-400'} dark:text-zinc-500`}>
                  <code>{t}</code>
                </dd>
              ))
            ) : (
              <dd className={`font-mono text-xs ${type === 'required' ? 'text-red-500' : 'text-zinc-400'} dark:text-zinc-500`}>
                <code>{type}</code>
              </dd>
            )}
          </div>
